...
class Drawing:
    ...
    def drawing_camera(self):
        ...
        text_surface = self.font.render(f"{self.camera.distance_to_object(self.player.pos):.2f}", True, (0, 0, 0))
    ...
    def drawing_player(self):
        pygame.draw.line(self.window,
                         (0, 0, 200),
                         self.player.pos,
                         (self.player.x + math.cos(self.player.angle) * 2 * self.player.radius,
                          self.player.y + math.sin(self.player.angle) * 2 * self.player.radius,))
        pygame.draw.circle(self.window, (0, 0, 200), self.player.pos, self.player.radius)